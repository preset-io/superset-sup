...
def list_groups(
    ...
    try:
        ...
        if save_file:
            save_groups_to_file(all_groups, save_file, team)
            if not porcelain:
                console.print(
            ...
            display_groups_table(all_groups, team)
...
def sync_groups(
    ...
    try:
        ...
        # Get existing groups
        existing_groups = get_all_groups(client, team)
        ...
        with spinner("Synchronizing groups", silent=porcelain) as sp:
            results = execute_group_sync(client, team, operations)
...
def create_group(
    ...
    try:
        with spinner(f"Creating group '{name}'", silent=porcelain):
            ...
            # Create the group
            response = create_scim_group(client, team, group_data)
...
def execute_group_sync(
    ...
    for op in operations["update"]:
        try:
            ...
            client.update_scim_group(team, op["id"], group_data)
            results["updated"] += 1
        except Exception as e:
    ...
    for op in operations["delete"]:
        try:
            client.delete_scim_group(team, op["id"])
            results["deleted"] += 1
        except Exception as e:
...
def create_scim_group(client, team: str, group_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return client.create_scim_group(team, group_data)